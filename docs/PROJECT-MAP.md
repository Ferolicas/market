...
- Frutas y verduras: cada mesa (`PRODUCE_DISPLAY_POSITIONS`) tiene cuatro cubetas inclinadas hacia la cámara, una por producto (`PRODUCE_BIN_COLUMNS`, orden de `RETAIL_DEPARTMENTS.produce.products`), y cada cubeta lleva su cartel (`retail-slot-sign:*`) con réplica del producto, nombre, `unidades/capacidad` de esa mesa y `faltan n` o `LLENO`. La cantidad dibujada es exactamente la autoritativa: las unidades y la capacidad (`shelfCapacityForTier`, motor) se reparten entre las mesas por turnos (`distributedFixtureQuantity`), una cubeta sin stock se ve vacía y no existe decoración fija. Los vuelos de reposición aterrizan en la mesa y hueco donde aparecerá la unidad (`retailStockFixtureSlot`). La cabecera del departamento cuelga del riel trasero para no tapar los carteles.
