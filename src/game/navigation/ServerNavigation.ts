...
import { NavMeshService, createWalkableStoreGeometry, walkableSignature } from "./NavMeshService";
 ...
 * baked meshes. The cache key is `walkableSignature()` — the same one the
 * client uses to invalidate its navmesh — not a raw join of area names, so
 * two unlocked-areas sets that leave every obstacle's availability unchanged
 * share one cached mesh instead of building an equivalent one twice.
 */
const MAXIMUM_CACHED_MESHES = 64;
const services = new Map<string, Promise<NavMeshService | null>>();
let wasmReady: Promise<void> | null = null;

function unlockedAreasOf(state: GameState): readonly string[] {
...
async function serviceFor(signature: string, areas: readonly string[]) {
...
export async function serverPathfinderFor(state: GameState): Promise<WorldPathfinder | undefined> {
  const areas = unlockedAreasOf(state);
  const service = await serviceFor(walkableSignature(areas), areas);
