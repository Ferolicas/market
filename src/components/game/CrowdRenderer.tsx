...
import { InstanceRegistry, PartsInstancer, type InstancedPart } from "@/game/render/CrowdParts";
import { CROWD_INSTANCE_ATTRIBUTE, createCrowdMaterial, createCrowdPoseAttribute, crowdBoneIndex, loadCrowdAnimation, readCrowdBoneMatrix, type CrowdAnimationSet } from "@/game/render/CrowdSkinning";
...
/**
 * Crowd rendering: every customer and employee body of one kind is a single
 * InstancedMesh skinned in the vertex shader from a baked bone texture, and
 * every prop they carry (carts, wheels, products, bags, baskets, hats) is an
 * instanced part list. Per character and frame the main thread computes a
 * position, a heading, a clip and a couple of socket matrices — no mixers,
 * no bone hierarchies, no per-character draw calls.
 */
const BODY_CAPACITY = 48;
const PROP_CAPACITY = 64;
const PRODUCT_CAPACITY = 320;

type CustomerId = CustomerRuntimeState["identity"];
const CUSTOMER_BODY_KEYS: Record<CustomerId, string> = {
...
const EMPLOYEE_BODIES: CharacterId[] = ["adult-woman", "adult-man", "adult-woman", "adult-man"];
const EMPLOYEE_BODY_KEYS: Partial<Record<CharacterId, string>> = { "adult-man": "owner_man", "adult-woman": "owner_woman" };
const BODY_SCALE: Record<CharacterId, number> = { "adult-man": 1.264, "adult-woman": 1.302, boy: 1.322, girl: 1.264 };
const HAT_FIT_SCALE: Record<CharacterId, number> = { "adult-man": 0.49, "adult-woman": 0.49, boy: 0.64, girl: 0.68 };
const HAT_FILES: Record<HatId, string> = {
...
const LOCOMOTION_CLIPS = new Set<CrowdClipName>(["Walk", "Run", "CarryWalk", "CarryRun", "BasketWalk", "Enter", "Exit"]);
...
type BodyRegistry = InstanceRegistry<CrowdBody>;
type PartsRegistry = InstanceRegistry<PartsInstancer>;

function nowMs() {
  return typeof performance === "undefined" ? Date.now() : performance.now();
}

// Crowd instances are not frustum-culled by three (one mesh spans the store),
// so the frame loop culls per character with a generous sphere and skips the
// pose, sockets and props of anyone off screen, as the mixers used to pause.
const frustum = new THREE.Frustum();
const frustumMatrix = new THREE.Matrix4();
const visibilitySphere = new THREE.Sphere(new THREE.Vector3(), 2.6);
function updateFrustum(camera: THREE.Camera) {
  frustumMatrix.multiplyMatrices(camera.projectionMatrix, camera.matrixWorldInverse);
  frustum.setFromProjectionMatrix(frustumMatrix);
}
function characterInView(x: number, z: number) {
  visibilitySphere.center.set(x, 1.1, z);
  return frustum.intersectsSphere(visibilitySphere);
}
...
function socketMatrix(animation: CrowdAnimationSet, joint: "Head" | "Hand_L" | "Hand_R", row: number, target: THREE.Matrix4) {
...
/** One body kind: loads its LOD mesh and baked animation, then registers an
 * InstancedMesh the frame loop fills. A `warm` batch instead draws one tiny
 * instance at `warm` so the program compiles and the atlas and bone texture
 * upload while the loading cover is still up. */
function CrowdBodyBatch({ bodyKey, modelPath, registry, parent, warm }: { bodyKey: string; modelPath: string; registry: BodyRegistry; parent: THREE.Group | null; warm?: readonly [number, number, number] }) {
  ...
  useEffect(() => {
    ...
    prepared.traverse((node) => { if (!skinned && node instanceof THREE.SkinnedMesh) skinned = node; });
    if (!skinned) return;
    const source = skinned as THREE.SkinnedMesh;
    ...
    const mesh = new THREE.InstancedMesh(geometry, createCrowdMaterial(baseMaterial, animation), warm ? 1 : BODY_CAPACITY);
    const pose = createCrowdPoseAttribute(warm ? 1 : BODY_CAPACITY);
    geometry.setAttribute(CROWD_INSTANCE_ATTRIBUTE, pose);
    mesh.name = `crowd-body:${bodyKey}${warm ? ":warm" : ""}`;
    mesh.count = 0;
    ...
    parent.add(mesh);
    registry.set(bodyKey, { mesh, pose, animation, count: 0 });
    return () => {
      registry.delete(bodyKey);
...
function DeliveredProductBatch({ id, productId, registry, parent }: { id: "milk" | "cheese" | "egg"; productId: ProductId; registry: PartsRegistry; parent: THREE.Group | null }) {
...
function HatBatch({ hat, body, registry, parent }: { hat: HatId; body: CharacterId; registry: PartsRegistry; parent: THREE.Group | null }) {
  const tier = useCharacterModelTier();
...
function usePropInstancers(parent: THREE.Group | null, definitions: Record<string, { parts: InstancedPart[]; capacity: number }>) {
  const registry = useMemo<PartsRegistry>(() => new InstanceRegistry(), []);
  useEffect(() => {
    if (!parent) return;
  ...
  }, [definitions, parent, registry]);
  return registry;
}

function productPartDefinitions() {
  const definitions: Record<string, { parts: InstancedPart[]; capacity: number }> = {};
  for (const productId of PRODUCT_IDS) {
    const parts = basketProductParts(productId);
    if (parts) definitions[`product:${productId}`] = { parts, capacity: PRODUCT_CAPACITY };
  }
  return definitions;
}

function pushProduct(products: PartsRegistry, delivered: PartsRegistry, productId: ProductId, matrix: THREE.Matrix4) {
...
export function CrowdCustomers({ customers }: { customers: readonly CustomerRuntimeState[] }) {
  const tier = useCharacterModelTier();
  const bodies = useMemo<BodyRegistry>(() => new InstanceRegistry(), []);
  const delivered = useMemo<PartsRegistry>(() => new InstanceRegistry(), []);
  const [parent, setParent] = useState<THREE.Group | null>(null);
  const propDefinitions = useMemo(() => ({
    shadow: { parts: GROUND_SHADOW_PARTS, capacity: PROP_CAPACITY },
    cart: { parts: CART_CHASSIS_PARTS, capacity: PROP_CAPACITY },
    caster: { parts: [CART_CASTER_PART], capacity: PROP_CAPACITY * 4 },
    wheel: { parts: [CART_WHEEL_PART], capacity: PROP_CAPACITY * 4 },
    cartBag: { parts: CART_BAG_PARTS, capacity: PROP_CAPACITY },
    handBag: { parts: HAND_BAG_PARTS, capacity: PROP_CAPACITY },
    ...productPartDefinitions(),
  }), []);
  const props = usePropInstancers(parent, propDefinitions);
  const actors = useRef(new Map<string, CustomerActor>());
  const identities = useMemo(() => [...new Set(customers.map((customer) => customer.identity))], [customers]);
  const deliveredIds = useMemo(() => [...new Set(customers.flatMap((customer) => Object.keys(customer.basket)).map((id) => deliveredProductId(id)).filter((id): id is "milk" | "cheese" | "egg" => Boolean(id)))], [customers]);
  const modelTier = tier === 2 ? 2 : 1;
  ...
  useFrame(({ clock, camera }, delta) => {
    ...
    for (const customer of liveActors.customers.values()) {
      ...
      let actor = actors.current.get(customer.id);
      if (!actor) {
        ...
        actors.current.set(customer.id, actor);
      }
  ...
  return <group ref={setParent} name="crowd:customers">
    {identities.map((identity) => <CrowdBodyBatch key={identity} bodyKey={CUSTOMER_BODY_KEYS[identity]} modelPath={characterModelPathForTier(`/models/market/customers/${CUSTOMER_BODY_KEYS[identity]}.glb`, modelTier)} registry={bodies} parent={parent} />)}
    {deliveredIds.map((id) => <DeliveredProductBatch key={id} id={id} productId={id === "egg" ? "eggs" : id} registry={delivered} parent={parent} />)}
  </group>;
}
...
export function CrowdEmployees({ employees }: { employees: readonly Employee[] }) {
  const tier = useCharacterModelTier();
  const bodies = useMemo<BodyRegistry>(() => new InstanceRegistry(), []);
  const hats = useMemo<PartsRegistry>(() => new InstanceRegistry(), []);
  const delivered = useMemo<PartsRegistry>(() => new InstanceRegistry(), []);
  const [parent, setParent] = useState<THREE.Group | null>(null);
  const propDefinitions = useMemo(() => ({
    shadow: { parts: GROUND_SHADOW_PARTS, capacity: PROP_CAPACITY },
    basket: { parts: HARVEST_BASKET_PARTS, capacity: PROP_CAPACITY },
    ...productPartDefinitions(),
  }), []);
  const props = usePropInstancers(parent, propDefinitions);
  const actors = useRef(new Map<string, EmployeeActor>());
  const modelTier = tier === 2 ? 2 : 1;
  const bodyOf = (index: number) => EMPLOYEE_BODIES[index % EMPLOYEE_BODIES.length];
  const bodyKinds = useMemo(() => [...new Set(employees.map((_, index) => bodyOf(index)))], [employees]);
  const hatKinds = useMemo(() => [...new Set(employees.map((employee, index) => `${bodyOf(index)}:${employee.hat}`))], [employees]);
  const deliveredIds = useMemo(() => [...new Set(employees.flatMap((employee) => Object.keys(employee.runtime?.carry.items ?? {})).map((id) => deliveredProductId(id)).filter((id): id is "milk" | "cheese" | "egg" => Boolean(id)))], [employees]);
  ...
  useFrame(({ camera }, delta) => {
    ...
    employees.forEach((employee, index) => {
      ...
      if (!actor) {
        ...
        actors.current.set(employee.id, actor);
      }
      actor.lastSeen = frameNow;
  ...
  return <group ref={setParent} name="crowd:employees">
    {bodyKinds.map((bodyId) => <CrowdBodyBatch key={bodyId} bodyKey={EMPLOYEE_BODY_KEYS[bodyId]!} modelPath={characterModelPathForTier(`/models/market/characters/${EMPLOYEE_BODY_KEYS[bodyId]}.glb`, modelTier)} registry={bodies} parent={parent} />)}
    {hatKinds.map((key) => { const [bodyId, hat] = key.split(":") as [CharacterId, HatId]; return <HatBatch key={key} hat={hat} body={bodyId} registry={hats} parent={parent} />; })}
    {deliveredIds.map((id) => <DeliveredProductBatch key={id} id={id} productId={id === "egg" ? "eggs" : id} registry={delivered} parent={parent} />)}
  </group>;
}
