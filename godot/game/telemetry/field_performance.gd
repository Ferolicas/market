...
func take() -> Dictionary:
	...
	var result := {"frameCount": frames.size(), "averageFrameMs": JS.round(average * 100) / 100.0, "p95FrameMs": JS.round(p95 * 100) / 100.0, "framesOver25": frames.filter(func(value): return value > 25).size(), "longTaskCount": long_tasks.size(), "maxLongTaskMs": JS.round(JS.max_of([0] + long_tasks) * 100) / 100.0}
	frames = []
	long_tasks = []
	return result
