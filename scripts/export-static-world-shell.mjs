// TEMPORARY standalone copy of export-static-world.mjs, scoped to /runtime only:
// bakes ONLY the structural shell (ground/city/building — confirmed unconditional,
// never gated by unlockedAreas) so WorldKit can render furniture/farm live on top
// without duplicate/stale geometry. Does not touch the shared script or /play2's
// existing level30.glb.
...
const dump = await page.evaluate(() => {
  ...
  const groups = ["perf:ground", "perf:city", "perf:building"];
